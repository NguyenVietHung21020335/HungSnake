...
PlayGround::PlayGround(int width, int height)
    : board(height, vector<CellType>(width, CELL_EMPTY)), //Khởi tạo ô vuông: dựa vào số dòng, số cột
...
void PlayGround::changeCellState(Position pos, CellType type)
{
    ...
    board[pos.y][pos.x] = type; //Truy nhập x và y từ struct pos -slide 23 file lec12
}

void PlayGround::processUserKeyboardInput(UserKeyboardInput input)
{
    snake.processUserKeyboardInput(input); // Hàm lấy từ PlayGround.h
}
